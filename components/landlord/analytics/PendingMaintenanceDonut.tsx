        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
            <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
                ...
                <button
                    ...
                    className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium text-white bg-emerald-600 active:bg-emerald-700 rounded-lg transition-colors"
                >
                    View All
                    ...
                    <div className="px-2 py-2 grid grid-cols-3 gap-1.5">
                        {STATUS_CONFIG.map((s) => {
                            ...
                            return (
                                <div
                                    key={s.key}
                                    className="rounded-lg px-1 py-1.5 text-center border active:opacity-80"
                                    style={{ backgroundColor: s.bg, borderColor: s.color + "33" }}
                                >
                                    <div className="flex items-center justify-center gap-0.5">
                                        <Icon className="w-2.5 h-2.5" style={{ color: s.text }} />
                                        <p className="text-xs font-bold" style={{ color: s.text }}>
                                            {counts[s.key]}
                                        </p>
                                    </div>
                                    <p
                                        className="text-[7px] leading-tight font-semibold uppercase mt-0.5"
                                        style={{ color: s.text }}
                                    >
                                        {s.key === "in-progress" ? "Progress" : s.label}
                                    </p>
                                </div>
                            );
                        })}
                        ...
                        {pendingLoading ? (
                            ...
                            <div className="space-y-2">
                                {pendingRequests.map((req: any) => (
                                    <div
                                        key={req.request_id}
                                        className="p-2.5 rounded-lg border border-gray-100 active:bg-gray-50"
                                    >
