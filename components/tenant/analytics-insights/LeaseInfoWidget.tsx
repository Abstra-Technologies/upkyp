...
export default function LeaseInfoWidget({ agreement_id }: Props) {
            ...
            <div className="space-y-2.5">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                        ...
                        <p className="text-xs font-bold text-gray-900">Lease Period</p>
                    </div>
                    <span className={`px-2 py-0.5 text-[10px] font-semibold rounded-full border ${statusColor}`}>
                        {leaseStatus}
                    </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                    <div className="bg-blue-50 border border-blue-100 rounded-md p-2">
                        <p className="text-[9px] text-blue-600 font-medium uppercase">Start</p>
                        <p className="text-xs font-bold">{startDate ? formatDate(startDate) : "-"}</p>
                    </div>
                    <div className={`rounded-md border p-2 ${statusColor}`}>
                ...
                {/* Financials - compact inline row */}
                <div className="flex items-center gap-3 text-xs">
                    <div className="flex items-center gap-1">
                        <CreditCardIcon className="w-3 h-3 text-blue-600" />
                        <span className="text-gray-500">Rent</span>
                        <span className="font-bold text-gray-900">{formatCurrency(lease.rent_amount || 0)}</span>
                    </div>
                    {Number(lease.security_deposit_amount) > 0 && (
                        <div className="flex items-center gap-1">
                            <ShieldCheckIcon className="w-3 h-3 text-gray-400" />
                            <span className="text-gray-500">Deposit</span>
                            <span className="font-bold text-gray-900">{formatCurrency(lease.security_deposit_amount)}</span>
                        </div>
                    )}
                    {Number(lease.advance_payment_amount) > 0 && (
                        <div className="flex items-center gap-1">
                            ...
                            <span className="font-bold text-gray-900">{formatCurrency(lease.advance_payment_amount)}</span>
                        </div>
                    )}
                </div>
