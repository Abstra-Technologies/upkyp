...
create table PlanUnitPriceByPropertyType
(
    id            int auto_increment
        primary key,
    plan_id       int                                         not null,
...
create index idx_plan_unit_price_plan_id
    on PlanUnitPriceByPropertyType (plan_id);

create index idx_plan_unit_price_property_type
    on PlanUnitPriceByPropertyType (property_type);
...
(
    config_id          char(36)                     not null
        primary key,
    property_id        varchar(12)                  not null,
    billingReminderDay tinyint unsigned             null,
    billingDueDay      tinyint unsigned             null,
    meterReadingDay    tinyint unsigned             null,
    notifyEmail        tinyint(1)                   null,
    notifySms          tinyint(1)                   null,
    lateFeeType        enum ('fixed', 'percentage') null,
    lateFeeFrequency   enum ('one_time', 'per_day') null,
    lateFeeAmount      decimal(10, 2)               null,
    gracePeriodDays    smallint unsigned            null,
...
create index idx_subtype_property_type
    on PropertySubtype (property_type);
...
(
    id                char(36)                                                 default (uuid())          not null
        primary key,
    landlord_id       varchar(20)                                                                        not null,
    didit_session_id  varchar(50)                                                                        null,
    didit_workflow_id varchar(50)                                                                        null,
...
(
    ...
    constraint fk_subscription_pending_plan
        foreign key (pending_plan_id) references Plan (plan_id)
            on update cascade,
...
create table SubscriptionMonthlyBillingSnapshot
(
...
create index idx_snapshot_cycle_id
    on SubscriptionMonthlyBillingSnapshot (xendit_cycle_id);

create table SubscriptionMonthlyBillingSnapshotItem
(
    ...
    constraint fk_snapshot_item_snapshot
        foreign key (snapshot_id) references SubscriptionMonthlyBillingSnapshot (snapshot_id)
            on delete cascade
);

create index idx_snapshot_item_snapshot_id
    on SubscriptionMonthlyBillingSnapshotItem (snapshot_id);
...
create index idx_payment_snapshot
    on SubscriptionPayment (snapshot_id);
